...
func test_stir_detects_circular_motion() -> String:
	...
	var points: Array[Vector2] = [Vector2(1, 0), Vector2(0, 1), Vector2(-1, 0), Vector2(0, -1), Vector2(1, 0)]
...
func test_pour_amount_uses_hold_time() -> String:
	var analyzer = load("res://src/core/gesture_analyzer.gd").new()
	var amount: float = analyzer.pour_amount(1.5, 2.0)
...
func test_whip_path_neatness_rewards_smooth_path() -> String:
	var analyzer = load("res://src/core/gesture_analyzer.gd").new()
	var path: Array[Vector2] = [Vector2(0, 0), Vector2(20, 3), Vector2(40, 5), Vector2(60, 4)]
	var score: float = analyzer.score_whip_path(path)
	if score < 0.8:
...
func test_short_inputs_return_zero() -> String:
	var analyzer = load("res://src/core/gesture_analyzer.gd").new()
	var short_stir_points: Array[Vector2] = [Vector2(1, 0)]
	var short_whip_points: Array[Vector2] = [Vector2(0, 0), Vector2(0, 0)]
	var empty_topping_points: Array[Vector2] = []
	if analyzer.score_stir(short_stir_points) != 0.0:
		return "Short stir input should score zero"
	if analyzer.score_whip_path(short_whip_points) != 0.0:
		return "Short whip input should score zero"
	if analyzer.score_topping_distribution(empty_topping_points, Rect2(0, 0, 10, 10)) != 0.0:
...
func test_pour_amount_handles_boundaries() -> String:
	...
	var repeated_whip_points: Array[Vector2] = [Vector2(0, 0), Vector2(0, 0), Vector2(1, 0), Vector2(2, 0)]
	var repeated_whip_score: float = analyzer.score_whip_path(repeated_whip_points)
...
func test_non_finite_inputs_return_safe_scores() -> String:
	var analyzer = load("res://src/core/gesture_analyzer.gd").new()
	var invalid_stir_points: Array[Vector2] = [Vector2(NAN, 0), Vector2(0, 1), Vector2(-1, 0), Vector2(0, -1), Vector2(1, 0)]
	var invalid_whip_points: Array[Vector2] = [Vector2(0, 0), Vector2(NAN, 0), Vector2(1, 0)]
	var invalid_topping_points: Array[Vector2] = [Vector2(NAN, 0), Vector2(1, 1)]
	...
	if analyzer.score_stir(invalid_stir_points) != 0.0:
		return "NaN stir point should return zero"
	if analyzer.score_whip_path(invalid_whip_points) != 0.0:
		return "NaN whip point should return zero"
	if analyzer.score_topping_distribution(invalid_topping_points, Rect2(0, 0, 10, 10)) != 0.5:
		return "NaN topping point should score as outside"
	return ""

func test_topping_distribution_counts_closed_edges() -> String:
	var analyzer = load("res://src/core/gesture_analyzer.gd").new()
	var points: Array[Vector2] = [Vector2(0, 0), Vector2(10, 10), Vector2(11, 10)]
	var score: float = analyzer.score_topping_distribution(points, Rect2(0, 0, 10, 10))
